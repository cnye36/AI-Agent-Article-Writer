...
export async function processArticleWritingJob(jobId: string): Promise<void> {
  ...
  try {
    ...
    await JobQueue.updateJobProgress(jobId, {
      current: 90,
      total: 100,
      message: "Running editor agent to refine content...",
    });

    // Run editor agent to refine the content
    try {
      const editorAgent = createEditorAgent();
      const editorResult = await editorAgent.invoke({
        articleContent: result.fullArticle,
        articleType: outline.article_type,
        tone: outline.tone,
        editedContent: "",
      });

      const editedContent = editorResult.editedContent || result.fullArticle;
      ...
      const { data: updatedArticle, error: updateError } = await supabase
        ...
        .update({
          ...
          content_html: await convertToHtml(editedContent),
          word_count: countWords(editedContent),
          reading_time: Math.ceil(countWords(editedContent) / 200),
        })
      ...
      if (!updateError && updatedArticle) {
        ...
        await supabase.from("article_versions").insert({
          ...
          edited_by: "ai",
          change_summary:
            "Content refined by AI editor agent (removed AI patterns, fixed em dashes, improved flow)",
        } as ArticleVersion);

        // Update saved article reference
        Object.assign(savedArticle, updatedArticle);
      }
    } catch (editorError) {
      console.error(
        "Editor agent failed, continuing with original content:",
        editorError
      );
      // Continue with original content if editor fails
    }
    ...
    const output: WriteArticleJobOutput = {
      ...
      metadata: {
        wordCount: savedArticle.word_count || wordCount,
        readingTime: savedArticle.reading_time || readingTime,
