...
export function ImagePreviewModal({
  ...
  // Debug: Log onEdit status
  useEffect(() => {
    console.log(
      "[ImagePreviewModal] onEdit provided:",
      !!onEdit,
      "isEditing:",
      isEditing,
      "isEditingState:",
  ...
  }, [onEdit, isEditing, isEditingState]);
    ...
    >
      <div className="relative max-w-7xl max-h-[90vh] w-full h-full flex items-center justify-center">
        ...
        <div className="absolute top-4 right-4 z-[110] flex gap-2 items-start">
          {/* Edit button - Always show if onEdit provided (only hide when in edit form mode) */}
          {onEdit && !isEditing && (
            <button
              onClick={(e) => {
                e.stopPropagation();
                e.preventDefault();
                console.log("[ImagePreviewModal] Edit button clicked");
        ...
        >
          {isEditing ? (
            ...
            <p
              className="text-sm text-zinc-300 truncate text-center"
              title={alt}
            >
              {alt}
            </p>
