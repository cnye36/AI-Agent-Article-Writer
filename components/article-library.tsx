    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-4">
        ...
        <div className="flex-1 min-w-[200px]">
          <input
            ...
            className="w-full bg-slate-50 dark:bg-zinc-800 border border-slate-300 dark:border-zinc-700 rounded-lg px-4 py-2 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-zinc-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
          />
        </div>
        ...
        <select
          ...
          className="bg-slate-50 dark:bg-zinc-800 border border-slate-300 dark:border-zinc-700 rounded-lg px-3 py-2 text-slate-900 dark:text-white focus:outline-none focus:ring-1 focus:ring-blue-500"
        >
      ...
      {selectedArticles.size > 0 && (
        <div className="bg-slate-50 dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-lg p-3 flex items-center gap-4">
          ...
          <button
            ...
            className="px-3 py-1 text-sm bg-green-600 hover:bg-green-500 rounded-lg text-white"
          >
            Publish
          </button>
          <button
      ...
      {isLoading && articles.length === 0 ? (
        ...
        <div className="text-center py-12">
          <p className="text-slate-600 dark:text-zinc-400 mb-2">No articles found</p>
      ...
      {pagination.hasMore && (
        <div className="flex justify-center">
          <button
            onClick={loadMore}
            disabled={isLoading}
            className="px-6 py-2 bg-slate-100 dark:bg-zinc-800 hover:bg-slate-200 dark:hover:bg-zinc-700 rounded-lg transition-colors disabled:opacity-50 text-slate-900 dark:text-white"
          >
  ...
  return (
    <div
      className={cn(
        ...
        isSelected ? "border-blue-500 shadow-md" : "border-slate-200 dark:border-zinc-800 hover:border-slate-300 dark:hover:border-zinc-700 hover:shadow-md dark:hover:shadow-none"
      )}
    >
      {/* Header */}
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-2">
          <input
            ...
            className="rounded bg-slate-100 dark:bg-zinc-800 border-slate-300 dark:border-zinc-700"
          />
        ...
        <div className="relative">
          <button
            ...
            className="p-1 hover:bg-slate-100 dark:hover:bg-zinc-800 rounded text-slate-600 dark:text-zinc-400"
          >
            ⋮
          </button>
          {showMenu && (
            <div className="absolute right-0 top-8 bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 rounded-lg shadow-xl py-1 min-w-[120px] z-10">
              <button
                onClick={() => { onEdit(); setShowMenu(false); }}
                className="w-full px-3 py-1.5 text-sm text-left hover:bg-slate-100 dark:hover:bg-zinc-700 text-slate-700 dark:text-zinc-300"
              >
                Edit
              </button>
              <button
              ...
              {article.status !== "published" && (
                <button
                  ...
                  className="w-full px-3 py-1.5 text-sm text-left hover:bg-slate-100 dark:hover:bg-zinc-700 text-green-600 dark:text-green-400"
                >
                  Publish
                </button>
              )}
              <button
                ...
                className="w-full px-3 py-1.5 text-sm text-left hover:bg-slate-100 dark:hover:bg-zinc-700 text-red-600 dark:text-red-400"
              >
                Delete
              </button>
            </div>
      ...
      {/* Title */}
      <h3
        className="font-semibold text-lg mb-2 line-clamp-2 cursor-pointer text-slate-900 dark:text-white hover:text-blue-600 dark:hover:text-blue-400"
        onClick={onEdit}
      >
        {article.title}
      </h3>
      ...
      {/* Meta */}
      <div className="flex items-center gap-3 text-xs text-slate-500 dark:text-zinc-500 mb-3">
